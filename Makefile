...
#
# This is the Makefile used to build PYTHIA examples on POSIX systems.
# Example usage is:
#     make main101
...
# Pythia8 and FastJet configuration
PYTHIA8      := /usr/local/pythia8
FASTJET      := /usr/local/fastjet
FASTJET_CONTRIB := /home/henfar/Downloads/fjcontrib-1.054
PYTHIA8_INC  := $(PYTHIA8)/include
PYTHIA8_LIB  := $(PYTHIA8)/lib
FASTJET_INC  := $(FASTJET)/include
FASTJET_CONTRIB_INC := $(FASTJET_CONTRIB)/include
FASTJET_LIB  := $(FASTJET)/lib

# Compiler
CXX          := g++
CXXFLAGS     := -std=c++17 -O2 -pedantic -W -Wall -Wshadow -fPIC $(ROOTCFLAGS) -I$(PYTHIA8_INC) -I$(FASTJET_INC) -I$(FASTJET_CONTRIB_INC)
LDFLAGS      := -Wl,-rpath,$(PYTHIA8_LIB) -Wl,-rpath,$(FASTJET_LIB) -Wl,-rpath,/home/henfar/root_install/lib
LDLIBS       := $(ROOTLIBS) -L$(PYTHIA8_LIB) -lpythia8 -L$(FASTJET_LIB) -lfastjet
